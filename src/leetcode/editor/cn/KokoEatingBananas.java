...
public class KokoEatingBananas{
    public static void main(String[] args) {
        ...
        int []input={3,6,7,11};
        System.out.println(solution.minEatingSpeed(input,8));
    }
...
class Solution {
    public int minEatingSpeed(int[] piles, int h) {
        ...
        int min=1;int max=maxInt;
        int left=min,right=max;
        int mid;
        while(left<right){
            mid=left+(right-left)/2;
            if(isValid(mid,piles,h)<=0){
                right=mid;
            }else{
                left =mid+1;
            }
    ...
    public int isValid(int speed,int[]piles, int h ){
            int count=0;
            for(int i:piles){
                count+=(i/speed)+((i%speed>0)?1:0);
            }
            if(count<h) return -1;
            else if(count==h)return 0;
            else return 1;
